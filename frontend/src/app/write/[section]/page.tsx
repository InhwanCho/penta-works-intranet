...
const emptyDraft = (): Draft => ({
  ...
  writtenAt: localDate(), hospitalName: "", hospitalId: "", modelName: "", serviceType: "", contractType: "", manufactureCountry: "", manufactureDate: "", manufacturer: "GE",
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (section === "repairs" && !restored.manufacturer.trim()) restored.manufacturer = "GE";
  ...
  return <div className="shell record-shell"><RecordSidebar activeSection={section} /><main className="write-page record-main">
    ...
    <section className="write-wrap">
      ...
      <form className="write-form" onSubmit={submit}>
        ...
        {section === "repairs" && <>
          <div className="form-section"><h2>기본 정보</h2><div className="form-grid"><label>작성일<input type="date" required value={draft.writtenAt} onChange={(event) => update("writtenAt", event.target.value)} /></label><label>등록 병원<select value={draft.hospitalId} onChange={(event) => { const id = event.target.value; update("hospitalId", id); const hospital = hospitals.find((item) => String(item.id) === id); if (hospital) update("hospitalName", hospital.name); }}><option value="">직접 입력</option>{hospitals.map((hospital) => <option key={hospital.id} value={hospital.id}>{hospital.name}</option>)}</select></label>{!draft.hospitalId && <label>병원명<input value={draft.hospitalName} onChange={(event) => update("hospitalName", event.target.value)} /></label>}<label>형명·모델명<input value={draft.modelName} onChange={(event) => update("modelName", event.target.value)} /></label><label>담당자<select value={draft.assigneeId} onChange={(event) => update("assigneeId", event.target.value)}><option value="">미지정</option>{users.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label><label>서비스 구분<select value={draft.serviceType} onChange={(event) => update("serviceType", event.target.value)}><option value="">선택 안 함</option><option value="PM">정기점검(PM)</option><option value="REPAIR">고장수리</option><option value="COLDHEAD">Cold Head</option><option value="ACR">ACR</option><option value="INSTALL">설치</option><option value="ETC">기타</option></select></label><label>계약 구분<select value={draft.contractType} onChange={(event) => update("contractType", event.target.value)}><option value="">선택 안 함</option><option value="C">C</option><option value="W">W</option><option value="On-call">On-call</option></select></label><label>제조국<input value={draft.manufactureCountry} onChange={(event) => update("manufactureCountry", event.target.value)} /></label><label>제조사<input value={draft.manufacturer} onChange={(event) => update("manufacturer", event.target.value)} /></label><label>제조년월일<input type="date" value={draft.manufactureDate} onChange={(event) => update("manufactureDate", event.target.value)} /></label></div></div>
          <div className="editor-field"><span>수리 내용</span><MarkdownEditor value={draft.content} onChange={(value) => update("content", value)} onUploaded={(id) => setDraft((old) => ({ ...old, fileIds: [...old.fileIds, id] }))} /></div>
          <details className="form-section form-disclosure"><summary>작업 정보 <small>필요할 때 펼쳐서 입력</small></summary><div className="form-disclosure-body"><div className="form-grid"><label>작업일<input type="date" value={draft.workDate} onChange={(event) => update("workDate", event.target.value)} /></label><label>작업 시작<input type="time" value={draft.workStartTime} onChange={(event) => update("workStartTime", event.target.value)} /></label><label>작업 종료<input type="time" value={draft.workEndTime} onChange={(event) => update("workEndTime", event.target.value)} /></label><label>교통시간(분)<input type="number" min="0" value={draft.travelMinutes} onChange={(event) => update("travelMinutes", event.target.value)} /></label></div><label>특기사항<textarea rows={3} value={draft.specialNotes} onChange={(event) => update("specialNotes", event.target.value)} /></label><label>부품 내역<textarea rows={4} placeholder="부품번호, 부품명, 수량, 단가, 금액 등을 입력하세요." value={draft.partsDetails} onChange={(event) => update("partsDetails", event.target.value)} /></label></div></details>
