  ...
  return <main className="write-page">
    ...
    <section className="write-wrap">
      ...
      <form className="write-form" onSubmit={submit}>
        ...
        <label>내용<MarkdownEditor value={draft.content} onChange={(value) => update("content", value)} onUploaded={(id) => update("fileIds", [...draft.fileIds, id])} /></label>
        {section === "notices" && <label className="check"><input type="checkbox" checked={draft.pinned} onChange={(event) => update("pinned", event.target.checked)} /> 상단 고정</label>}
