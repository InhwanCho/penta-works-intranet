...
export default function HospitalFormPage() {
  ...
  useEffect(() => {
    void Promise.all([api<Me>("/auth/me"), editing ? api<Hospital>(`/hospitals/${params.id}`) : Promise.resolve(null)])
      .then(([me, row]) => {
        ...
        if (row) {
          setName(String(row.name ?? "")); setCode(String(row.code ?? "")); setRegion(String(row.region ?? ""));
          setAddress(String(row.address ?? "")); setNotes(String(row.notes ?? "")); setPmIntervalMonths(String(row.pm_interval_months ?? 6));
          setContacts(readArray<Contact>(row.contacts_json)); setSystems(readArray<System>(row.systems_json));
        }
        setReady(true);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const payload = { name, code, region, address, notes, pmIntervalMonths: Number(pmIntervalMonths) || 6,
